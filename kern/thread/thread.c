...
thread_bootstrap(void)
{
	...
	stopped = cv_create("stopped");
	stoplock = lock_create("stoplock");
	    ...
	    pid_t *ret)
{
	...
	alloc = pid_alloc(&newthread->t_pid);
	if (alloc) {
		pid_unalloc(newthread->t_pid);
		thread_destroy(newthread);	// clean
		return alloc;
	}


	addresspace = curthread->t_addrspace;
	newaddresspace = &newthread->t_addrspace;
	if (addresspace != NULL) {
		result = as_copy(addresspace, newaddresspace);
		if(result) {
			pid_unalloc(newthread->t_pid);
